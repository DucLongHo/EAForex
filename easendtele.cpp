...
int OnInit(){
    ...
    if(EnableNotifications){
        ...
        startMessage += "⏰ Timeframe: " + TimeToString(TimeCurrent(), TIME_DATE|TIME_SECONDS) + "\n";
    ...
    for(int index = PositionsTotal() - 1; index >= 0; index--){
        ...
        if(PositionSelectByTicket(ticket)){
            ...
            if(TimeCurrent() - positionTime <= 10){
                ...
                message += "Thời gian: " + TimeToString(positionTime, TIME_DATE|TIME_MINUTES);
...
void ProcessNewDeal(ulong dealTicket){
    if(HistoryDealGetInteger(dealTicket, DEAL_ENTRY) == DEAL_ENTRY_OUT){
        ...
        message += "Thời gian: " + TimeToString(HistoryDealGetInteger(dealTicket, DEAL_TIME), TIME_DATE|TIME_MINUTES);
